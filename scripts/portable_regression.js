...
// --- 4b. native first-run template neutrality ---------------------------------
// The JS DEFAULTS above cover the retired app's defaults. The native bar writes
// its OWN starting file (g_template in native/src/p_ui.c, read as JSONC by the
// C parser), and it must ship the same neutrality: the guard that examined only
// DEFAULTS is what let a template with the token master on and personal
// session stores enabled through. Decode the C string, strip the JSONC
// comments and assert meaning from the parsed object.
{
